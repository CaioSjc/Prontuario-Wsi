    ...
    return (
      <>      
        <E.ContentNavBar>
          ...
          <E.ContentImg2 src={HeaderTeste} /><E.ContentListMarginApp>Aplicação de Teste</E.ContentListMarginApp>
        ...
        <E.ContentSalvarPular>
          ...
          <E.ButtonSalvar onClick={() => Navigate('/ObsComportamental')} >
            ...
            <E.ButtonPular onClick={() => Navigate('/ObsComportamental')} >
              ...
              <T.ButtonAddTeste>  
              <T.ContentOptionsAdd>
              <T.ContentStyleAdd src={Add} /><T.OptionDisable disabled selected>Adicionar</T.OptionDisable><T.ContentStyleExpand  src={Expand} />
              <p></p>
              <T.StyleOptionAtençao onClick={() => renderClick('ModalAppTeste')} >Atenção Concentrada/Seletiva</T.StyleOptionAtençao>
                 <p></p>
              <T.StyleOptionMemoria onClick={() => renderClick('ModalAppMemoria')} >Memória</T.StyleOptionMemoria>
                <p></p>
              <T.StyleOptionInteligencia onClick={() => renderClick('ModalAppInteligencia')} >Inteligência</T.StyleOptionInteligencia>            
              </T.ContentOptionsAdd>        
              </T.ButtonAddTeste>
              {isModalOpen && isNameModalOpen === 'ModalAppTeste' && (
                 <ModalAppTeste isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} > 
                 </ModalAppTeste>
                 )}
              {isModalOpen && isNameModalOpen === 'ModalAppMemoria' && (
                <ModalAppMemoria isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} > 
                </ModalAppMemoria>
                )}
                 {isModalOpen && isNameModalOpen === 'ModalAppInteligencia' && ( 
                <ModalAppInteligencia isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}> 
